...
namespace YourEasyBot
{
	public class EasyBot	// A fun way to code Telegram Bots, by Wizou
	{
		...
		public async Task<Document> NewTxtFile(UpdateInfo update, CancellationToken ct = default)
		{
			while (await NewMessage(update, ct) != MsgCategory.MediaOrDoc) { }
			...
			while (true)
			{
				switch (await NextEvent(update, ct))
				{
					case UpdateKind.CallbackQuery:
						if (update.CallbackData == "cancel")
						{
							await Telegram.EditMessageTextAsync(msg.Chat.Id, msg.MessageId, "Главное меню.", replyMarkup: Keyboards.mainMenu);
			...
			while (true)
			{
				switch (await NextEvent(update, ct))
				{
					...
					case UpdateKind.CallbackQuery:
						...
						if (update.CallbackData == "cancel")
						{
							await Telegram.EditMessageTextAsync(update.Message.Chat.Id, update.Message.MessageId, "Главное меню.", replyMarkup: Keyboards.mainMenu);
							throw new Exception("cancel");
						}
